import { c as createComponent, r as renderTemplate, d as renderHead, e as renderScript, g as renderComponent, m as maybeRenderHead } from '../../chunks/astro/server_vBMvLF1I.mjs';
import 'kleur/colors';
...
const $$Koleksi = createComponent(async ($$result, $$props, $$slots) => {
  return renderTemplate(_a || (_a = __template(['<html lang="en"> <head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Koleksi Manager Supabase</title><!-- Tailwind CSS via CDN atau build pipeline -->', "", `</head> <div class="flex flex-col lg:flex-row gap-6"> <!-- Daftar koleksi --> <div id="koleksi-container" class="flex-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4"> <!-- Akan di-render JS --> </div> <!-- Side form untuk edit saja --> <div id="edit-panel" class="w-full lg:w-80 bg-white rounded-lg shadow p-4 hidden"> <h3 class="text-lg font-medium text-gray-800 mb-4">Edit Koleksi</h3> <form id="koleksi-form" class="flex flex-col space-y-3"> <!-- Hidden field untuk menyimpan id yang diedit --> <input type="hidden" id="koleksi-id"> <div> <label for="koleksi-nama" class="block text-sm font-medium text-gray-700">Nama Koleksi</label> <input type="text" name="nama" id="koleksi-nama" class="mt-1 w-full border border-gray-300 rounded px-2 py-1 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Nama koleksi"> </div> <div> <label for="koleksi-gambar" class="block text-sm font-medium text-gray-700">Ganti Gambar (opsional)</label> <input type="file" accept="image/*" name="gambar" id="koleksi-gambar" class="mt-1 w-full"> </div> <div> <label class="block text-sm font-medium text-gray-700">Preview Gambar</label> <img id="koleksi-preview" src="" alt="Preview" class="w-full h-32 object-cover rounded border mt-1 bg-gray-50"> </div> <div class="flex justify-end space-x-2 pt-2"> <button type="button" id="btn-cancel" class="px-3 py-1 bg-gray-300 rounded hover:bg-gray-400">Batal</button> <button type="submit" class="px-3 py-1 bg-green-500 text-white rounded hover:bg-green-600">Simpan</button> </div> </form> </div> </div> <!-- Script Supabase & logic --> <script type="module">
  ...
  <\/script> </html>`], ['<html lang="en"> <head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Koleksi Manager Supabase</title><!-- Tailwind CSS via CDN atau build pipeline -->', "", `</head> <div class="flex flex-col lg:flex-row gap-6"> <!-- Daftar koleksi --> <div id="koleksi-container" class="flex-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4"> <!-- Akan di-render JS --> </div> <!-- Side form untuk edit saja --> <div id="edit-panel" class="w-full lg:w-80 bg-white rounded-lg shadow p-4 hidden"> <h3 class="text-lg font-medium text-gray-800 mb-4">Edit Koleksi</h3> <form id="koleksi-form" class="flex flex-col space-y-3"> <!-- Hidden field untuk menyimpan id yang diedit --> <input type="hidden" id="koleksi-id"> <div> <label for="koleksi-nama" class="block text-sm font-medium text-gray-700">Nama Koleksi</label> <input type="text" name="nama" id="koleksi-nama" class="mt-1 w-full border border-gray-300 rounded px-2 py-1 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Nama koleksi"> </div> <div> <label for="koleksi-gambar" class="block text-sm font-medium text-gray-700">Ganti Gambar (opsional)</label> <input type="file" accept="image/*" name="gambar" id="koleksi-gambar" class="mt-1 w-full"> </div> <div> <label class="block text-sm font-medium text-gray-700">Preview Gambar</label> <img id="koleksi-preview" src="" alt="Preview" class="w-full h-32 object-cover rounded border mt-1 bg-gray-50"> </div> <div class="flex justify-end space-x-2 pt-2"> <button type="button" id="btn-cancel" class="px-3 py-1 bg-gray-300 rounded hover:bg-gray-400">Batal</button> <button type="submit" class="px-3 py-1 bg-green-500 text-white rounded hover:bg-green-600">Simpan</button> </div> </form> </div> </div> <!-- Script Supabase & logic --> <script type="module">
    import { createClient } from 'https://cdn.jsdelivr.net/npm/@supabase/supabase-js/+esm';
  ...
  <\/script> </html>`])), renderScript($$result, "Z:/WEB/kp-dyesablon-main/src/components/admin/Koleksi.astro?astro&type=script&index=0&lang.ts"), renderHead());
}, "Z:/WEB/kp-dyesablon-main/src/components/admin/Koleksi.astro", void 0);

const $$Index = createComponent(($$result, $$props, $$slots) => {
  return renderTemplate`${renderComponent($$result, "Layout", $$Layoutadmin, { "title": "Koleksi" }, { "default": ($$result2) => renderTemplate` ${maybeRenderHead()}<main> ${renderComponent($$result2, "AdminForm", $$Koleksi, {})} </main> ` })}`;
}, "Z:/WEB/kp-dyesablon-main/src/pages/admin/koleksi/index.astro", void 0);

const $$file = "Z:/WEB/kp-dyesablon-main/src/pages/admin/koleksi/index.astro";
const $$url = "/admin/koleksi";
